...
export default function Footer({ data: propData, className, contentMaxWidth }: FooterProps) {
  ...
  return (
    <div className={`relative overflow-hidden rounded-t-3xl bg-secondary ${className}`}>
      ...
      <div className={`relative mx-auto ${contentMaxWidth || 'max-w-9xl'} px-6 py-14 sm:px-10 sm:py-20 lg:px-10`}>
          ...
          <div className="md:w-[38%] md:shrink-0">
            ...
            <div className="mt-6">
              ...
              <div className="mt-3 flex gap-3 justify-center md:justify-start">
                <a href={data.socialLinks.tiktok} target="_blank" rel="noopener noreferrer" aria-label="TikTok" className="rounded-xl flex h-10 w-10 items-center justify-center bg-white text-secondary transition-opacity hover:opacity-90" onClick={() => trackEvent({ event: 'social_click', event_category: 'social', event_label: 'tiktok_footer' })}>
                ...
                <a href={data.socialLinks.facebook} target="_blank" rel="noopener noreferrer" aria-label="Facebook" className="rounded-xl flex h-10 w-10 items-center justify-center bg-white text-secondary transition-opacity hover:opacity-90" onClick={() => trackEvent({ event: 'social_click', event_category: 'social', event_label: 'facebook_footer' })}>
                  <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" /></svg>
                </a>
                <a href={data.socialLinks.zalo} target="_blank" rel="noopener noreferrer" aria-label="Zalo" className="rounded-xl flex h-10 w-10 items-center justify-center bg-white text-secondary transition-opacity hover:opacity-90" onClick={() => trackEvent({ event: 'social_click', event_category: 'social', event_label: 'zalo_footer' })}>
            ...
            <div className="mt-6 text-center md:text-start">
              <Link
                to="/van-ban-phap-ly"
                className="text-xs font-medium text-white/70 underline underline-offset-2 transition-colors hover:text-white"
              >
                Văn bản pháp lý
              </Link>
            </div>

            <p className="mt-3 text-xs text-center md:text-start text-white/50">
              {data.copyright}
            </p>
          </div>
            ...
            <div className="rounded-2xl bg-white px-8 py-8 sm:px-10 sm:py-10">
              ...
              <form className="mt-8 space-y-5" onSubmit={async (e) => {
                ...
                try {
                  ...
                  trackEvent({ event: 'form_submit', event_category: 'lead', event_label: 'contact_form' })
                  form.reset()
